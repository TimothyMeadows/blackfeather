...
namespace Blackfeather.Data
{
    ...
    public sealed class ManagedMemory : IDisposable
    {
        ...
        public void Delete(string pointer, string name)
        {
            ...
            var memorySet = _memory.FirstOrDefault(entry => entry.Pointer == pointer && entry.Name == name);
            if (memorySet.Equals(default(ManagedMemorySpace)))
            {
                return;
            }

            _memory.Remove(memorySet);
        ...
        public string ToJson(string pointer = null)
        {
            ...
            var json = JsonConvert.SerializeObject(memoryFilter.ToArray(), Formatting.Indented);
        ...
        public void FromJson(string value, bool append = false)
        {
            ...
            var memorySpaces = JsonConvert.DeserializeObject<ManagedMemorySpace[]>(value);
