...
namespace Blackfeather.Security.Cryptography
{
    ...
    public class Contract
    {
        ...
        public SignedContract Sign(UnsignedContract contract, string signature)
        {
            ...
            signed.Hash =
                $"participants={signed.Participants}&private={signed.Private}&public={signed.Public}&signatures={string.Join(":", signed.Signatures.Select(t => t.Hash).ToArray())}&expires={signed.Expires}"
        ...
        {
            var suspected =
                $"participants={contract.Participants}&private={contract.Private}&public={contract.Public}&signatures={string.Join(":", contract.Signatures.Select(t => t.Hash).ToArray())}&expires={contract.Expires}"
                    .ToHmac(contract.Private.FromHex()).ToHex();
